...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        predicted_y = nn.AddBias(nn.Linear(first_layer, self.w1), self.b1)
        return predicted_y
...
class DigitClassificationModel(object):
    ...
    def train(self, dataset):
        ...
        while True:
            ...
            for x, y in dataset.iterate_once(self.batch_size):
                loss = self.get_loss(x, y)
                grad = nn.gradients(loss, [self.w0, self.b0, self.w1, self.b1, self.w2, self.b2])
                self.w0.update(grad[0], -self.learning_rate)
                self.b0.update(grad[1], -self.learning_rate)
                self.w1.update(grad[2], -self.learning_rate)
                self.b1.update(grad[3], -self.learning_rate)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.b_hidden = nn.Parameter(1, 400)
        self.w1 = nn.Parameter(400, 5)
    ...
    def run(self, xs):
        ...
        """
        "*** YOUR CODE HERE ***"
        h_initial = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.w0), self.b0))
        h = h_initial
        for index in range(1, len(xs)):
            h = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(xs[index], self.w0), nn.Linear(h, self.w_hidden)), self.b_hidden))
        last_layer = nn.AddBias(nn.Linear(h, self.w1), self.b1)
        return last_layer
    ...
    def train(self, dataset):
        ...
        while True:
            for xs, y in dataset.iterate_once(self.batch_size):
                ...
                grad = nn.gradients(loss, [self.w0, self.b0, self.w_hidden, self.b_hidden, self.w1, self.b1])
                self.w0.update(grad[0], self.learning_rate)
                self.b0.update(grad[1], self.learning_rate)
                self.w_hidden.update(grad[2], self.learning_rate)
                self.b_hidden.update(grad[3], self.learning_rate)
                self.w1.update(grad[4], self.learning_rate)
                self.b1.update(grad[5], self.learning_rate)
